...
class LRSchedulerStep(object):
    ...
    def __init__(self, fastai_optimizer: OptimWrapper, total_step, lr_phases, mom_phases):
        self.optimizer = fastai_optimizer
...
class OneCycle(LRSchedulerStep):
    ...
    def __init__(self, fastai_optimizer, total_step, lr_max, moms, div_factor, pct_start):
        self.lr_max = lr_max
        self.moms = moms
        ...
        lr_phases = (
            (0, partial(annealing_cos, low_lr,
                        self.lr_max)), (self.pct_start, partial(annealing_cos, self.lr_max, low_lr / 1e4))
        )
        mom_phases = (
            (0, partial(annealing_cos, *self.moms)), (self.pct_start, partial(annealing_cos, *self.moms[::-1]))
        )
        fastai_optimizer.lr, fastai_optimizer.mom = low_lr, self.moms[0]
        super().__init__(fastai_optimizer, total_step, lr_phases, mom_phases)
