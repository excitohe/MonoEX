...
from monoex.utils.model_serialization import load_state_dict
from monoex.utils.model_zoo import cache_url
...
class Checkpointer():
    ...
    def load(self, f=None, use_latest=True):
        if self.has_checkpoint() and use_latest:
            # override argument with exist checkpoint
            f = self.get_checkpoint_file()
        if not f:
            # no checkpoint could be found
            self.logger.info("No checkpoint found. Init model from scratch")
            return {}
...
class DetectronCheckpointer(Checkpointer):
    ...
    def _load_file(self, f):
        if f.startswith("catalog://"):
            paths_catalog = import_file("monoex.config.paths_catalog", self.cfg.PATHS_CATALOG, True)
