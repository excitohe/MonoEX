...
class ContiguousParams:
    ...
    def _init_buffers(self):
        ...
        if not all(p["params"][0].dtype == dtype for p in self._parameters):
            raise ValueError("All parameters must be the same dtype.")
        if not all(p["params"][0].device == device for p in self._parameters):
            raise ValueError("All parameters must be the same device.")
    ...
    def make_params_contiguous(self):
        """
        Create a buffer to hold all params and update the params 
        to be views of the buffer.
        """
    ...
    def assert_buffer_is_valid(self):
        if not self.buffer_is_valid():
            raise ValueError(
                "The data or grad buffer has been invalidated. Please make "
