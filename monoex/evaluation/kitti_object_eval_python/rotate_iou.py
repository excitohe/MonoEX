...
def sort_vertex_in_convex_polygon(int_pts, num_of_inter):
    if num_of_inter > 0:
        center = cuda.local.array((2, ), dtype=nb.float32)
        center[:] = 0.0
        ...
        vs = cuda.local.array((16, ), dtype=nb.float32)
        for i in range(num_of_inter):
...
def line_segment_intersection(pts1, pts2, i, j, temp_pts):
    A = cuda.local.array((2, ), dtype=nb.float32)
    B = cuda.local.array((2, ), dtype=nb.float32)
    C = cuda.local.array((2, ), dtype=nb.float32)
    D = cuda.local.array((2, ), dtype=nb.float32)
...
def quadrilateral_intersection(pts1, pts2, int_pts):
    ...
    for i in range(4):
        ...
        if point_in_quadrilateral(pts2[2 * i], pts2[2 * i + 1], pts1):
            ...
            num_of_inter += 1
    temp_pts = cuda.local.array((2, ), dtype=nb.float32)
    for i in range(4):
        for j in range(4):
...
def rbbox_to_corners(corners, rbbox):
    ...
    x_d = rbbox[2]
    y_d = rbbox[3]
    corners_x = cuda.local.array((4, ), dtype=nb.float32)
    corners_y = cuda.local.array((4, ), dtype=nb.float32)
    corners_x[0] = -x_d / 2
    corners_x[1] = -x_d / 2
    corners_x[2] = x_d / 2
    corners_x[3] = x_d / 2
...
def inter(rbbox1, rbbox2):
    corners1 = cuda.local.array((8, ), dtype=nb.float32)
    corners2 = cuda.local.array((8, ), dtype=nb.float32)
    intersection_corners = cuda.local.array((16, ), dtype=nb.float32)
...
def rotate_iou_kernel_eval(N, K, dev_boxes, dev_query_boxes, dev_iou, criterion=-1):
    ...
    block_boxes = cuda.shared.array(shape=(64 * 5, ), dtype=nb.float32)
    block_qboxes = cuda.shared.array(shape=(64 * 5, ), dtype=nb.float32)
...
def rotate_iou_gpu_eval(boxes, query_boxes, criterion=-1, device_id=0):
    ...
    (take 5ms in one example with nb.cuda code).
