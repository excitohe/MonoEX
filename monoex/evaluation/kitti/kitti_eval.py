...
KITTI_ID_TYPE_CONVERSION = {0: 'Car', 1: 'Cyclist', 2: 'Pedestrian'}


def kitti_evaluation(eval_type, dataset, predictions, output_folder):
    ...
    if "detection" in eval_type:
        ...
        result_dict = do_kitti_detection_evaluation(
            dataset=dataset, predictions=predictions, output_folder=output_folder, logger=logger
        )
        return result_dict
...
def do_kitti_detection_evaluation(dataset, predictions, output_folder, logger):
    ...
    PathManager.mkdirs(predict_folder)
    ...
    with open(predict_txt, 'w', newline='') as f:
        ...
        if len(prediction) == 0:
            ...
            for p in prediction:
                p = p.numpy()
                p = p.round(4)
                type = KITTI_ID_TYPE_CONVERSION[int(p[0])]
