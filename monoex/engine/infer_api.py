...
from monoex.engine.visual_api import show_image_with_boxes
from monoex.evaluation import evaluate_python, generate_kitti_3d_detection
from monoex.utils import comm
from monoex.utils.timer import Timer, get_time_str
        ...
        for idx, batch in enumerate(tqdm(data_loader)):
            ...
            if vis:
                show_image_with_boxes(
                    vis_target.get_field('ori_img'),
                    output,
                    vis_target,
                    visualize_preds,
                    vis_scores=eval_utils['vis_scores']
                )
...
def inference(
    ...
    dis_ious = compute_on_dataset(model, data_loader, device, predict_folder, inference_timer, vis, eval_score_iou)
    comm.synchronize()
    ...
    logger.info('Finish generating predictions and start evaluating ...')
    ret_dicts = []

    for metric in metrics:
        result, ret_dict = evaluate_python(
            label_path=dataset.label_dir,
            result_path=predict_folder,
            label_split_file=dataset.imageset_txt,
            current_class=dataset.class_names,
            metric=metric
        )
...
def inference_all_depths(
    ...
    eval_depth_methods = [
        'oracle', 'hard', 'soft', 'mean', 'direct', 'keypoints_center', 'keypoints_02', 'keypoints_13'
    ]
    ...
    for depth_method in eval_depth_methods:
        ...
        dis_ious = compute_on_dataset(
            model, data_loader, device, method_predict_folder, inference_timer, vis, eval_score_iou
        )
        result, ret_dict = evaluate_python(
            label_path=dataset.label_dir,
