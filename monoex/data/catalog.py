...
from monoex.utils.logger import log_first_n
...
class DatasetCatalog(object):
    ...
    e.g. "coco_2014_train") to a function which parses the dataset and returns 
    the samples in the format of `list[dict]`.

    The returned dicts should be in Detectron2 Dataset format (See DATASETS.md 
    for details) if used with the data loader functionalities in `data/build.py` 
    and `data/detection_transform.py`.

    The purpose of having this catalog is to make it easy to choose different 
    datasets, by just using the strings in the config.
    """
    ...
    def register(name, func):
        ...
        Args:
            ...
            func (callable): a callable which takes no arguments and returns a list 
                of dicts. It must return the same results if called multiple times.
        """
...
class Metadata(types.SimpleNamespace):
    ...
    # the name of the dataset set default to N/A so that `self.name` in the errors
