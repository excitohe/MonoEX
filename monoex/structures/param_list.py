import torch
from monoex.data.datasets.kitti_utils import Calibration
...
class ParamList():
    ...
    def to(self, device):
        target = ParamList(self.size, self.is_train)
    ...
    def __len__(self):
        if self.is_train:
            reg_num = len(torch.nonzero(self.extra_fields["tgt_fill_mask"]))
        else:
            reg_num = 0
        return reg_num
